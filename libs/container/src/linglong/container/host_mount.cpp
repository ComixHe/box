...
bool do_mount_with_fd(const std::string &root, const std::string &source,
  ...
  if (fd < 0) {
    logErr() << "fail to open destination " << destination
             << linglong::utils::errnoString();
    return false;
  }

  linglong::utils::defer closeFd{[fd] { ::close(fd); }};
  ...
  logErr() << "mount" << source << "to" << realPathStr
           << "failed:" << linglong::utils::RetErrString(ret)
...
bool HostMount::remount(const std::filesystem::path &target, uint32_t flags,
  ...
  if (::statfs(target.c_str(), &sfs) < 0) {
    logErr() << "statfs file" << target << " " << utils::errnoString();
    return false;
  }
  ...
  if ((flags | remount_flags) != flags) {
    ...
    if ((sfs.f_flags & MS_RDONLY) != 0) {
      ...
      if (::mount(nullptr, target.c_str(), nullptr, flags | remount_flags,
                  data_ptr) == -1) {
        logErr() << "failed to remount" << target << utils::errnoString();
        return false;
      }
    }
  }
...
bool HostMount::MountNode(const utils::Mount &m) {
  std::error_code ec;
  ...
  switch (sourceStatus.type()) {
    ...
    case std::filesystem::file_type::symlink: {
      ...
      if ((m.extensionFlags & utils::Extension::COPY_SYMLINK) != 0U) {
      ...
      if (nosymfollow) {
        ...
        if (sourceFd < 0) {
          logErr() << utils::format("fail to open source(%s):", source.c_str())
                   << utils::errnoString();
          return false;
        }
  ...
  auto data = utils::str_vec_join(m.data, ',');
  auto real_data = data;
  ...
  switch (m.fsType) {
    ...
    case utils::Mount::Proc:
    case utils::Mount::Devpts:
      [[fallthrough]];
    case utils::Mount::Tmpfs: {
      if (do_mount_with_fd(containerRoot, source, destination, m.type,
                           real_flags, real_data.c_str())) {
        return true;
      }
    } break;
    case utils::Mount::Mqueue: {
      if (do_mount_with_fd(containerRoot, source, destination, m.type,
                           real_flags, real_data.c_str())) {
        return true;
      }
  ...
  if (auto str = source.string(); !str.empty() && str.at(0) == '/') {
    ...
    linglong::utils::debug::DumpFileInfo(source);
  }

  linglong::utils::debug::DumpFileInfo(destination);
  return false;
}
