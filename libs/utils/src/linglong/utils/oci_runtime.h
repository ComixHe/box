...
#include "linglong/utils/json.h"
#include "linglong/utils/util.h"

// Compatible with linux kernel which is under 5.10
// ll-box will open the symlink with O_PATH | O_NOFOLLOW and replace source by
// '/proc/self/fd/${fd}' impl: method MountNode in file host_mount.cpp
...
#define LLJS_FROM_OPT_VARNAME(KEY, VAR) \
  (o.VAR = optional<decltype(o.VAR)::value_type>(j, #KEY))

#define LLJS_TO(KEY) (LLJS_TO_VARNAME(KEY, KEY))
#define LLJS_TO_VARNAME(KEY, VAR) (j[#KEY] = o.VAR)

#define LLJS_FROM_OBJ(TYPE) \
...
inline void from_json(const nlohmann::json &j, Process &o) {
  o.args = j.at("args").get<str_vec>();
  o.env = j.at("env").get<str_vec>();
...
inline void from_json(const nlohmann::json &j, Mount &o) {
  const static std::map<std::string_view, Mount::Type> fsTypes = {
      {"bind", Mount::Bind},     {"proc", Mount::Proc},
      {"devpts", Mount::Devpts}, {"mqueue", Mount::Mqueue},
      {"tmpfs", Mount::Tmpfs},   {"sysfs", Mount::Sysfs},
  ...
  auto options = j.value("options", str_vec{});
  for (auto const &opt : options) {
    if (auto it = optionFlags.find(opt);
        it != optionFlags.cend() && it->second.flag != 0) {
      if (it->second.clear) {
        o.flags &= ~it->second.flag;
      } else {
        o.flags |= it->second.flag;
      }
    } else if (auto it = propagationFlags.find(opt);
               it != propagationFlags.cend()) {
      o.propagationFlags |= it->second;
    } else if (auto it = extensionFlags.find(opt);
               it != extensionFlags.cend()) {
      if (it->second.clear) {
...
inline void to_json(nlohmann::json &j, const Mount &o) {
  ...
  j["source"] = o.source;
  j["type"] = o.type;
  j["options"] = o.data;  // FIXME: this data is not original options, some of
                          // them have been prased to flags.
}
...
static std::map<std::string, int> namespaceType = {
    ...
    {"mount", CLONE_NEWNS},    {"cgroup", CLONE_NEWCGROUP},
    {"network", CLONE_NEWNET}, {"ipc", CLONE_NEWIPC},
    {"user", CLONE_NEWUSER},
};
...
inline void to_json(nlohmann::json &j, const Namespace &o) {
  auto matchPair =
      std::find_if(std::begin(namespaceType), std::end(namespaceType),
...
inline void from_json(const nlohmann::json &j, Syscall &o) {
  o.names = j.at("names").get<str_vec>();
...
inline static Runtime fromFile(const std::string &filepath) {
  return json::fromFile(filepath).get<Runtime>();
}

inline static Runtime fromString(const std::string &content) {
  return json::fromByteArray(content).get<Runtime>();
}
